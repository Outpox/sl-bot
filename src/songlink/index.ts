...
export class SonglinkClient {
    ...
    private getSonglinkApiUrl(query: number | URL): string {
        if (typeof query === 'number') {
            return `https://api.song.link/v1-alpha.1/links?id=${query}&platform=appleMusic&type=song&userCountry=US&key=${process.env.SL_API_KEY}`
        } else {
            return `https://api.song.link/v1-alpha.1/links?url=${query.href}&userCountry=US&key=${process.env.SL_API_KEY}`
        }
    }
}
